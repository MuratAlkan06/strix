...
- A "raw" `unscopedDb` exists for genuinely cross-user or lifecycle operations — explicitly named to make grep-for-leaks easy. **CI check** (`scripts/check-unscoped-db.mjs`, five layers, fails the build on violation):
  - **Layer 1** — `unscopedDb` imports allowed only in `lib/inngest/*`, `app/api/webhooks/*`, `lib/auth/lifecycle.ts` (the soft-delete + login-recovery module, which must see soft-deleted users), and `db/scoped.integration.test.ts` (Phase 1's env-gated live-DB test — fixture user lifecycle + residue checks only). Additions to this allowlist are made in that script only — never weaken the rule inline at a call site.
