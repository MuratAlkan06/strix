...
Strix is Latin for *owl* (see DECISIONS — "Naming"). The mark is a **minimal geometric owl emblem ONLY** *(the in-app mark was aligned to the V6a owl-head on 2026-07-19 — see the dated note at the end of this section; the round-1 abstract-silhouette description below records the original seed the icon curation started from)*.

- **Construction:** flat-fill geometric SVG in the DAWN silhouette language (the same grammar as terrain scenes). *Round-1 seed:* abstract, no expressive face/eyes/limbs. *Current in-app mark (2026-07-19):* the V6a owl-head — tufted head silhouette with two eyes and a beak kite (still flat-fill, no strokes, amber as the single accent).
...
**App icon (minted 2026-06-12).** The PWA/home-screen icon is **V6a "Night Watch — flat"**: a tufted owl-head silhouette with two solid amber eye discs (`primary`) and an amber beak kite, on the flat `--background` dusk ground (`#0a1121`) — user-curated over three rounds on `/playground/icons` (round 1 emblem-derived, round 2 owl-forward, round 3 V6 ground refinements; the route is kept as the curation record). The icon stays grammar-native (flat fills, no strokes, amber as the single point of heat) but leads with the owl signifiers (eyes, tufts) rather than the emblem seed geometry, which reads as a blob at 60px. *(This paragraph originally recorded a deliberate icon↔in-app divergence — v6a icon, round-1 emblem in-app. That divergence was **superseded on 2026-07-19**: the in-app mark now also follows v6a — see the dated note below.)* **Design-system delta (icon-only, not a minted token):** the head fill is elevated dusk `oklch(0.26 0.04 264)` → `#1a2438` because `--card` (`0.225 L`) lacks separation against `--background` at icon scale (fails the 60px squint test, verified at true size). Generated by `scripts/generate-icons.mjs` (`WIRED_VARIANT = "v6a"`); the manifest and layout reference only the canonical `icon-*` / `apple-touch-icon-*` filenames. Decision recorded in [`DECISIONS.md`](DECISIONS.md) "Visual register".

**In-app mark aligned to v6a (2026-07-19).** At the v0.5.0 device demo the project lead flagged that the app still rendered the round-1 perched-owl seed emblem and decided the in-app brand mark must follow the curated **V6a "Night Watch"** owl — mark and icon should be one owl. `src/components/emblem.tsx` now renders the v6a head/eye/beak geometry (copied verbatim from `scripts/generate-icons.mjs`), superseding the earlier divergence; the four call sites (`horizon-header`, landing `page`, `empty-dashboard`, `~offline`) are unchanged (same `treatment`/`className` API). **Tonal lockup — why the in-app fills differ from the icon.** The icon is a large opaque tile on its own flat dusk ground, so it uses v6a's dark elevated-dusk head + solid amber eyes legibly at ≥60px. The in-app mark is a small (~24–40px) *bare* silhouette painted over live dawn/dusk scenes (sky-top L 0.16–0.20) and the offline `--background` — where a dark head vanishes and solid amber eyes wash out (verified). So in-app it uses the **dark-surface lockup of the same owl**: a light head silhouette (`--emblem-body`/`currentColor`) + dusk eye-sockets (`--background`) with amber irises + an amber beak — the treatment round-3 explored as "v6b", keeping amber the single point of heat and clearing the ≥3:1 non-text-glyph floor (§11). Same geometry, context-appropriate tones — not a second mark. **Seed-grammar (§12):** the in-app seed is now the v6a owl-head; whether the future owl-form construction derives from this head or the retired perched silhouette is left to design review. **Resolved (design review, 2026-07-19):** the v6a owl-head is adopted as the canonical owl-form construction seed — icon and in-app mark are one geometry, and any future owl-form system (§12) derives from this head, not the retired perched silhouette.
