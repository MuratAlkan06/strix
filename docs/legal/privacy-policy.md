...
**Usage analytics.** We measure how the product is used via PostHog, and the basis differs by where the event is recorded. **Server-side product events** — for example signup, goal created, plan generated, plan accepted, task checked off, check-in completed, and subscription started or canceled — are sent from our servers against your account identifier; they set no cookie and store nothing on your device, and we rely on the contract and legitimate-interests bases in §5. **Browser-side analytics** — intake-funnel events measured in your browser, together with device and browser metadata — load only after you opt in, because they use analytics cookies (§8): until you accept, the analytics SDK does not load, so no request is made and no cookie is set.
...
- **Restriction, objection, and consent withdrawal.** You can withdraw analytics consent in Settings at any time — the Analytics toggle stops the browser-side analytics and clears the analytics cookies from your device immediately. Server-side product events (§2) set no cookie and run under the bases in §5, so the toggle does not cover them; to object to or restrict that processing, contact us. For health-data consent, restriction, or objection requests, contact us at **[PLACEHOLDER: contact email]**.
