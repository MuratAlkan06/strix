 ...
 *
 * Draft handling: on first landing a goal_drafts row is created and an HttpOnly
 * cookie is set; a returning visit resumes the existing transcript.
 *
...
export default async function GoalIntakePage({ searchParams }: PageProps) {
  ...
  const { seed: rawSeed } = await searchParams;
