/**
 * INTEGRATION TEST — plan-prompt caching proof (env-gated; live Anthropic API).
 *
 * Mirrors caching.integration.test.ts for the plan system prompt. Skips
 * cleanly when ANTHROPIC_API_KEY is unset (CI, key-less dev). When a key IS
 * present:
...
import { planOutputFormat } from "./plan-schema";

const hasKey = Boolean(process.env.ANTHROPIC_API_KEY);
const run = hasKey ? describe : describe.skip;

run("plan prompt caching (integration)", () => {
  it("the cached system block clears the 1024-token cache floor", async () => {
    ...
    console.info(
      `plan system prompt tokens (incl. tiny user turn): ${count.input_tokens}`,
    );
  ...
  it("second identical-prefix call reads the cache", async () => {
    ...
    console.info(
      `plan cache: first creation=${first.usage.cache_creation_input_tokens} ` +
